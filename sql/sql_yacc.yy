 ...
 */
bool LEX::setup_select_in_parentheses()
{
  DBUG_ASSERT(current_select->braces);
  if (current_select->linkage == UNION_TYPE &&
      !current_select->master_unit()->first_select()->braces &&
      current_select->master_unit()->first_select()->linkage ==
      UNION_TYPE)
  {
    thd->parse_error();
    return TRUE;
...
select_paren:
          ...
          opt_select_lock_type
          {
            if (Lex->setup_select_in_parentheses())
              MYSQL_YYABORT;
          }
        | '(' select_paren ')'
        ;
